...
PetLink 是一款供固定五位好友使用的 Windows 联机桌宠 MVP。每位用户拥有一个个人房间，桌宠可以进入在线好友的房间；同一房间中的所有客户端同步桌宠集合、归一化位置、动作和交互状态，不传输真实桌面画面。
...
- 上传 PNG/JPEG/WebP 后自动去除简单纯色背景，并拆分为 `root/body/head` 轻量骨架。
...
要求：Node.js 18.18+、Rust 1.87+、Windows 10/11 和 WebView2。
...
- 自动图片处理是本地轻量算法，适合透明图或纯色背景；复杂照片建议先使用项目 Skill 生成合规透明源图。
